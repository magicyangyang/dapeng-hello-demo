...
import com.dapeng.example.config.{Database, DatabaseSupport}
import com.dapeng.example.entity.Blog
import com.dapeng.example.infrastructure.BlogRepository
  ...
  describe("selectMap") {
    it("should fetch records as Map") {
      withReadOnly(Database.default) { implicit session =>
        ...
        assert("title1" === blogs("title1"))
        assert("title2" === blogs("title2"))
  ...
  describe("selectMapBy") {
    it("should fetch records which starts with new prefix as Map") {
      withReadOnly(Database.default) { implicit session =>
        ...
        assert("newtitle2" === blogs("newtitle2"))
      }
    }
  }
  ...
  describe("selectMapPageBy") {
    it("should fetch records as Map by specifying title, limit, and offset") {
      withReadOnly(Database.default) { implicit session =>
        ...
        println(blogs)
        assert("newtitle2" === blogs("newtitle2"))
        assert("newtitle4" === blogs("newtitle4"))
        assert("newtitle6" === blogs("newtitle6"))
